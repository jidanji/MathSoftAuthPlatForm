...
namespace Business
{
    public class BLL_Math_ZZTARGET: BaseMathRoleAuthorEntities
    {
        private DbSet<Math_ZZTARGET> contextItem = null;
        public BLL_Math_ZZTARGET():base()
        {
        ...
        public UIModelData<Math_ZZTARGET> Add(Guid Id, string zhanzhangdaqu, string school, string zhanzhang, Expression<Func<Math_ZZTARGET, bool>> perWhere)
        {
        ...
        public List<Math_ZZTARGET> Search(int PageIndex, int PageSize, out int total, Expression<Func<Math_ZZTARGET, bool>> where = null)
        {
            ...
            IQueryable<Math_ZZTARGET> lazyList = contextItem.Where(exp1);
            return (PageIndex >= 0) ? lazyList.Take(PageSize).ToList() : lazyList.ToList();
        }
