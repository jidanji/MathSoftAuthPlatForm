...
namespace Business
{
    public class BLL_User: BaseMathRoleAuthorEntities
    {
        ...
        public UIModelData<Math_UserInfo> Add(string UserName, string UserAccount, string UserPhone, string UserTel, string UserPwd, Guid User_Dept_Id, string User_Sex, int User_Area, string RoleIds, Expression<Func<Math_UserInfo, bool>> perWhere)
        {
            ...
            if (total > 0)
            {
                ...
                uIModelData.remark = string.Format("登录名重复，注册失败，请更换登录名");
            }
            else
            {
        ...
        /// <returns></returns>
        public List<Math_UserInfo> Search(int PageIndex, int PageSize, out int total, Expression<Func<Math_UserInfo, bool>> where)
        {
        ...
        /// <param name="where"></param>
        /// <returns></returns>
        public List<Math_UserInfo> SearchComon(int PageIndex, int PageSize, out int total, Expression<Func<Math_UserInfo, bool>> where = null)
        {
            Guid admin = Guid.Parse("8C8CF1D5-0257-4EE1-ABD6-935A07C413CE");
            Expression<Func<Math_UserInfo, bool>> exp1 = null;

            Expression<Func<Math_UserInfo, bool>> exp2 = item => item.Math_User_Role_Select.Count(item1 => item1.RoleId != admin) > 0;
            ...
            total = contextItem.Count(exp1.And(exp2));
            IQueryable<Math_UserInfo> lazyList = contextItem
                    .Include("Math_User_Role_Select")
                    .Include("Math_User_Role_Select.Math_UserInfo")
                    .Where(exp1.And(exp2))
                    .OrderByDescending(item => item.UserOrderId);
