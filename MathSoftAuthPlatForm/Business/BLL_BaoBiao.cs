...
namespace Business
{
    ...
    /// 报表分析类
    /// </summary>
    public class BLL_BaoBiao: BaseMathRoleAuthorEntities
    {
        #region 构造函数
        /// <summary>
        /// 构造函数
        /// </summary>
        public BLL_BaoBiao():base()
        {
        }
        #endregion
        ...
        /// <returns></returns>
        public List<UIStatic> Search(string UserName, string searchBegDate, string searchEndDate, string searchDept, string searchStaticType)
        {
            string p1 = " and 1=1", p2 = " and 1=1", p3 = " and 1=1";
            //开始日期
...
SELECT  
 ...
 order by  TotalNumber desc";
  ...
  t2.TotalNumber as intotal,
 CAST((CAST(t2.TotalNumber as FLOAT)/CAST(t1.TotalNumber as FLOAT)*100 )as int ) as rate
...
SELECT  
      ...
      UserDeptName
  FROM  [dbo].[Math_Student]) t
 group by  UserId,UserName,UserDeptName) t1
 join (
 select  UserName
      ,UserId,
      UserDeptName,
...
SELECT  
      ...
      UserId,
      UserDeptName
  FROM  [dbo].[Math_Student] where IsIn='1') t 
