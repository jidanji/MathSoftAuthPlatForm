...
namespace Business
{
    public class BLL_Math_JYT: BaseMathRoleAuthorEntities
    {
        private DbSet<Math_JYT> contextItem = null;
        public BLL_Math_JYT():base()
        {
        ...
        public UIModelData<Math_JYT> Add(string StudentName, string IdCard, string StudentSex, string GrandSchool, DateTime InsertTime, DateTime LastUpdateTime, string DataFrom, string ZSLB, Expression<Func<Math_JYT, bool>> perWhere)
        {
        ...
        public List<Math_JYT> Search(int PageIndex, int PageSize, out int total, Expression<Func<Math_JYT, bool>> where = null)
        {
            ...
            IQueryable<Math_JYT> lazyList = contextItem.Where(exp1).OrderByDescending(item => item.InsertTime);
