...
namespace Business
{
    public class BLL_Math_Student: BaseMathRoleAuthorEntities
    {
        private DbSet<Math_Student> contextItem = null;

        public BLL_Math_Student():base()
        {
            contextItem = context.Math_Student;
        }

        public UIModelData<Math_Student> Add(string StudentName,string StudentIDCard, string StudentPhone, string StudentType, string StudentSchool
, Guid UserId
...
, Expression<Func<Math_Student, bool>> perWhere)
        ...
        public void SetStudentIn(string StudentIDCard)
        {
            MathRoleAuthorEntities context = new MathRoleAuthorEntities();
            DbSet<Math_Student> contextItem = context.Math_Student;

            Math_Student model = contextItem.FirstOrDefault(item => item.StudentIDCard.Trim() == StudentIDCard);
            if (model == null)
            {
        ...
        public IQueryable<Math_Student> DeleteByTeacher(Guid Teacher)
        {
            IQueryable<Math_Student> list = contextItem.Where(item => item.UserId == Teacher);
            if (list != null)
            {
                contextItem.RemoveRange(list);
                context.SaveChanges();
            }
            return list;
        }
        ...
        public List<Math_Student> Search(int PageIndex, int PageSize, out int total, Expression<Func<Math_Student, bool>> where = null)
        {
            ...
            IQueryable<Math_Student> lazyList = contextItem.Where(exp1).OrderByDescending(item => item.StudentOrderId);
