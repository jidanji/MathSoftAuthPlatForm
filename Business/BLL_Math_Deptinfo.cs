...
namespace Business
{
    public class BLL_Math_Deptinfo
    {
        ...
        public UIModelData<Math_Deptinfo> Update(
        Guid Math_Dept_Id
        , string Math_Dept_Name
        , string Math_Dept_Remark
        , Expression<Func<Math_Deptinfo, bool>> perWhere)
        {
            UIModelData<Math_Deptinfo> uIModelData = new UIModelData<Math_Deptinfo> { };
            int total = GetTotal(perWhere);
            if (total > 0)
        ...
        public List<Math_Deptinfo> Search(int PageIndex, int PageSize, out int total, Expression<Func<Math_Deptinfo, bool>> where = null)
        {
            ...
            if (PageIndex >= 0)
            {
                return contextItem.Where(exp1).OrderByDescending(item => item.Math_SeqNo).Skip(PageIndex).Take(PageSize).ToList();
            }
            else
            {
                return contextItem.Where(exp1).OrderByDescending(item => item.Math_SeqNo).ToList();
            }
        }
