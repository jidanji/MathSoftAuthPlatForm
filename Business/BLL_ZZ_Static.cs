    ...
    public class BLL_ZZ_Static
    {
      ...
      , string ZhanZhang)
        ...
        public List<ZZ_Static> Search(int PageIndex, int PageSize, out int total, Expression<Func<ZZ_Static, bool>> where = null)
        {
            ...
            if (PageIndex >= 0)
            {
                return contextItem.Where(exp1).OrderByDescending(i => i.ZhanZhangDaQu).Skip(PageIndex).Take(PageSize).ToList();
            }
            else
            {
                return contextItem.Where(exp1).OrderByDescending(i => i.ZhanZhangDaQu).ToList();
            }
        }
...
SELECT  count(*) Value,ZhanZhangDaQu,ZhanZhang
  FROM [MathRoleAuthor].[dbo].[ZZ_Static]
  where 
